...
const Sidebar = () => {
    ...
    return <aside className="w-80 min-h-screen bg-white border-r border-gray-100 flex flex-col fixed left-0 top-0">
            ...
            menuItems.map((item, index) =>
        {
            ...
            return (
                <Link href={item.link} key={index} className={`flex gap-3 items-center py-3 px-4.5 rounded-lg font-medium duration-300 ${isActive ? "bg-primary/15 text-primary" : "hover:bg-gray-200"}`}>  
                <item.icon size={24}/>
                <span>{item.name}</span>
                </Link>
            )
        ...
        <button className="flex cursor-pointer gap-3 font-medium py-3 px-4.5 mx-5 hover:bg-gray-200 duration-300 rounded-lg mt-auto mb-10" onClick={handleLogout}>
        <FiLogOut size={24}/>
        Logout
        </button>
    </aside>;
